...
def serialize(run, include_result=False):
    ...
    data["excel_available"] = bool(run.excel_path and Path(run.excel_path).is_file())
    return data


def create_run(*, idempotency_key, input_type, input_text=None, filename=None, content=None):
    init_database()
    ...
    with SessionLocal() as session:
        existing = session.scalar(select(GenerationRun).where(GenerationRun.idempotency_key == normalized_key))
        if existing:
        ...
        run = GenerationRun(id=run_id, idempotency_key=normalized_key, input_fingerprint=fingerprint,
...
def list_runs(limit=20):
    with SessionLocal() as session:
        return list(session.scalars(select(GenerationRun).order_by(GenerationRun.created_at.desc()).limit(limit)))
...
def finish_run(run_id, worker_token, result, excel_path):
    ...
    with SessionLocal.begin() as session:
        result = session.execute(update(GenerationRun).where(
            ...
            excel_path=str(excel_path), error_message=None, completed_at=now, updated_at=now,
            worker_token=None))
        return result.rowcount == 1
